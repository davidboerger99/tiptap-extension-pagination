...
import type { NullableNodePos } from "../../types/node"
...
import type { ParagraphLineInfo } from "../../types/paragraph"
...
import { isListItemNode } from "./listItem"
...
/**
 * Get the parent list item node position and the list item node itself.
 *
 * @param doc - The document node.
 * @param pos - The resolved position in the document or the absolute position of the node.
 * @returns {NullableNodePos} The position and the node or null if not found of the list item.
 */
export const getListItemNodeAndPosition = (doc: PMNode, pos: ResolvedPos | number): NullableNodePos => {
  if (typeof pos === "number") {
    return getListItemNodeAndPosition(doc, doc.resolve(pos))
  }

  const listItemPos = getParentNodePosOfType(doc, pos, "listItem")
  const listItemNode = doc.nodeAt(listItemPos.pos)
  if (!isListItemNode(listItemNode)) {
    console.warn("No list item node found")
    return { pos: -1, node: listItemNode }
  }

  return { pos: listItemPos.pos, node: listItemNode }
}

/**
 * Check if the position is within a list item.
 *
 * @param $pos - The resolved position in the document.
 * @returns {boolean} True if the position is within a list item, false otherwise.
 */
export const isPositionWithinListItem = ($pos: ResolvedPos): boolean => {
...
export const isPosAtLastLineOfParagraph = (
  view: EditorView,
  $pos: ResolvedPos | number,
): { isAtLastLine: boolean } & ParagraphLineInfo => {
