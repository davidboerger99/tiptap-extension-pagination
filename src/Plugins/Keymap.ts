...
const KeymapPlugin = keymap({
  ...
  ArrowRight: (state, dispatch) => {
    ...
    const { pos: nextParagraphPos, node: nextParagraphNode } = getFirstParagraphInNextPageBodyAfterPos(
      doc,
      paragraphPos,
    )
  ...
  ArrowUp: (state, dispatch, view) => {
    ...
    const cursorOffset =
      getOffsetForDistanceInLine(view, previousParagraphPos, prevParagraphLastLineNum, offsetDistance) + 1
  ...
  Enter: (state, dispatch) => {
    ...
    if (isPositionWithinListItem($pos)) {
      // Let the default list behavior handle it
      return false
    }
